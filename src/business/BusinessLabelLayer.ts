...
export class BusinessLabelLayer {
  ...
  public applyStyle(params: DebugParams['station']): void {
    for (const entry of this.entries) {
      entry.element.style.fontSize = `${params.labelFontSize}px`;
  ...
  public update(camera: THREE.Camera, width: number, height: number, params: DebugParams['station']): void {
    ...
    for (const candidate of candidates) {
      ...
      const forced = candidate.entry.route?.id === this.hoveredRoute?.id;
  ...
  private createRouteLabels(): void {
    for (const route of ROUTES) {
      ...
      element.textContent = formatRouteMetric(route);
      const object = new CSS2DObject(element);
  ...
  private updateAnchors(): void {
    ...
    for (const entry of this.entries) {
      ...
      const height = this.terrain.sampleUv(u, v) * this.terrain.meta.sceneUnitsPerMeter * this.exaggeration;
      const arc = entry.route?.mode === 'radial'
        ? 0.42 * Math.min(1, Math.hypot(
          (stations.get(entry.route.to)?.u ?? u) - u,
          (stations.get(entry.route.to)?.v ?? v) - v,
        ) * 20)
        : 0;
      entry.object.position.set(
        (u - 0.5) * this.terrain.meta.sceneWidth,
        height + STATION_ANCHOR_LIFT + arc,
    ...
    for (const entry of this.entries) {
      ...
      if (entry.route) {
        const visible = entry.route.mode === this.mode
          && (entry.route.id === this.hoveredRoute?.id || entry.route.mode === 'radial');
        entry.object.visible = visible;
        entry.element.style.display = visible ? '' : 'none';
        entry.element.classList.toggle('is-suppressed', !visible);
